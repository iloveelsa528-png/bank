...
const GroupSchema = {
  ...
  properties: {
    ...
    area:              { type: 'string', description: '문학, 독서, 문법, 화작, 기타 중 하나. 독서=설명문·논설문(과학·기술·경제·법·사회·인문 등 지식 전달); 문학=시·소설·수필·희곡 등 문학 작품; 문법=음운·어휘·문장·어문 규범; 화작=발표·토의·연설·고쳐쓰기. 무역·계약·법률·경제·과학·기술 지문은 반드시 독서.' },
...
[구조화 규칙]
- passageContent: 순수한 지문 본문만 남긴다. 아래 항목은 반드시 제거한다:
  (a) "[11~14] 다음 글을 읽고 물음에 답하시오." 형태의 지시문 전체
  (b) "12.", "13." 처럼 숫자(1~2자리)+마침표로 시작하는 발문 줄 전체
  (c) ①②③④⑤ 기호로 시작하는 선택지 줄 전체
  (d) "윗글에 대한 설명으로", "이 글의 내용과 일치하지 않는", "다음 중" 같이 문제 발문임이 명확한 어구로 시작하는 줄
  (e) "답: ③", "정답 ②" 같은 정답 표시 줄
  (f) [보기] 블록 중 문항에 종속된 것(문항 번호 다음에 오는 보기); 지문 내용을 직접 인용한 보기는 유지
  → 위 항목이 제거된 뒤 남은 본문 텍스트만 passageContent에 넣는다
  → 지문이 아예 없는 경우(문항만 있는 경우)는 passageContent를 빈 문자열로 반환
- choices: ①②③④⑤ 또는 1)2)3) 형태 모두 인식
- 지문 없이 문항만 있는 경우 passageContent는 빈 문자열

[영역(area) 분류 규칙]
- 독서: 설명문·논설문 등 정보 전달·주장 글. 과학·기술·사회·경제·법·예술·인문 주제의 지식 설명 지문.
  예) 무역계약, 비엔나 협약, 정형 거래 조건, 옥신 호르몬, 매체 철학, 경제 원리, 물리·화학 개념 등
- 문학: 시·시조·소설·수필·희곡 등 문학 갈래(서정·서사·극)의 실제 작품.
  판단 기준: 작품명·작가가 있고 서정적·서사적 언어를 사용한 경우.
- 문법: 음운·단어·문장 구조·어문 규범 등 국어 지식 자체를 묻는 문항. 지문 없이 문항만 있는 경우 多.
- 화작: 발표문·토의문·연설문(화법), 또는 초고·고쳐쓰기·개요(작문).
  판단 기준: "다음은 ○○이 발표한 내용입니다" 또는 "다음은 학생의 초고입니다" 형태.
- 기타: 위 4개에 명확히 속하지 않는 경우만.
- 주의: 전문 분야 설명 지문(무역·경제·법률·과학·기술 등)은 내용이 어렵더라도 독서이며, 절대 문학으로 분류하지 않는다.
