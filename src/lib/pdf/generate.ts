...
// Shared inline styles injected into each section's <html>
const BASE_CSS = `
...
function sectionDoc(bodyContent: string): string {
  return `<!DOCTYPE html><html lang="ko"><head><meta charset="UTF-8"/>
  <style>${BASE_CSS}</style></head><body>${bodyContent}</body></html>`;
}
  ...
  return sectionDoc(`
    <div style="padding:28px 40px 0">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px">
        <h1 style="font-size:19px;font-weight:800;color:#111827;line-height:1.4;flex:1">
          ${data.title}
        </h1>
        <span style="background:${modeBg};color:#fff;padding:5px 16px;border-radius:4px;
          font-size:12px;font-weight:700;white-space:nowrap;flex-shrink:0;margin-top:2px">
          ${modeLabel}
        </span>
      </div>
      <div style="margin-top:6px;font-size:12px;color:#6b7280;display:flex;justify-content:space-between">
        <span>${meta}</span>
        <span>생성일: ${date}</span>
      </div>
      <div style="margin-top:14px;border-bottom:2.5px solid #111827"></div>
    </div>
  `);
}
...
export function buildPassageSection(data: PdfData, mode: PdfMode): string {
            ...
            .map(
              (url) =>
                `<img src="${url}" crossorigin="anonymous"
                  style="max-width:100%;max-height:380px;display:block;margin-bottom:8px;
  ...
  const keyPointsHtml =
    mode === "full" && data.keyPoints
      ? `<div style="margin-top:14px;padding:10px 14px;border-top:1px dashed #d1d5db;
  ...
  return sectionDoc(`
    <div style="padding:18px 40px 0">
      <div style="border:2px solid #1e3a8a;border-radius:4px;overflow:hidden;">
        <div style="background:#1e3a8a;padding:10px 20px;display:flex;align-items:center;gap:10px;">
        ...
        <div style="padding:20px 24px;">
          ${imagesHtml}
          ${data.passageText
            ? `<p style="font-size:14px;color:#111827;line-height:2.1;
...
export function buildQuestionSection(
            ...
            .map((c, ci) => {
              ...
              const reasonHtml =
                showAnswerInfo && c.reason
                  ? `<div style="font-size:11px;color:#6b7280;margin-top:2px;padding-left:20px;">${c.reason}</div>`
                  : "";
              return `
              <div style="padding:7px 12px;border-radius:3px;
                ...
                <div style="display:flex;gap:8px;align-items:flex-start;">
                  <span style="font-weight:700;color:${highlight ? "#16a34a" : "#111827"};
                    flex-shrink:0;font-size:14px;">${circle}${mark}</span>
                  <span style="color:${highlight ? "#166534" : "#111827"};flex:1;font-size:14px;">${c.text}</span>
                </div>
                ${reasonHtml}
              </div>`;
            })
  ...
  const answerHtml =
    showAnswerInfo && q.answer > 0
      ? `<div style="margin-top:10px;padding:8px 14px;background:#f0fdf4;
            border-left:3px solid #16a34a;font-size:13px;color:#166534;font-weight:700;">
          정답: ${q.answer}번
        </div>`
      : "";
  ...
  const descriptiveHtml =
    showAnswerInfo && q.answer === 0 && q.descriptive_answer
      ? `<div style="margin-top:10px;padding:12px;background:#eff6ff;
            border-left:3px solid #3b82f6;font-size:13px;color:#1e40af;">
  ...
  const explanationHtml =
    showAnswerInfo && q.explanation
      ? `<div style="margin-top:10px;padding:12px;background:#fefce8;
            border-left:3px solid #eab308;font-size:12px;color:#713f12;">
          <strong>해설</strong><br/>
  ...
  const patternRef =
    mode === "full" && q.pattern_reference
      ? `<div style="font-size:11px;color:#7c3aed;margin-top:4px;">
          패턴 참조: ${q.pattern_reference}
        </div>`
      : "";

  return sectionDoc(`
    <div style="padding:16px 40px 0;">
      <div style="padding:18px 20px;border:1px solid #d1d5db;border-radius:6px;">
        <div style="display:flex;gap:10px;align-items:flex-start;">
          <span style="font-size:17px;font-weight:800;color:#111827;flex-shrink:0;">
            ${q.question_number}.
          </span>
          <div style="flex:1;">
            <p style="font-size:14px;font-weight:600;color:#111827;line-height:1.75;word-break:keep-all;">
...
export function buildHtml(data: PdfData, mode: PdfMode): string {
  ...
  return `<!DOCTYPE html><html lang="ko"><head><meta charset="UTF-8"/>
  <style>${BASE_CSS}body{padding:0;}</style></head>
